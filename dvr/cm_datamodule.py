...
def cache_paths_for_ct(project_root: str, ct_path: str) -> tuple[str, str, str]:
    """
    Generate cache paths for CT video, XR image, and text prompt.
    
    Returns:
        Tuple of (video_path, xr_path, prompt_path)
    """
    import hashlib
    stem = hashlib.sha1(os.path.abspath(ct_path).encode("utf-8")).hexdigest()
    videos_dir = os.path.join(project_root, "dvr", "cache", "videos")
    images_dir = os.path.join(project_root, "dvr", "cache", "images")
    prompts_dir = os.path.join(project_root, "dvr", "cache", "prompts")
    os.makedirs(videos_dir, exist_ok=True)
    os.makedirs(images_dir, exist_ok=True)
    os.makedirs(prompts_dir, exist_ok=True)
    return (
        os.path.join(videos_dir, f"{stem}.mp4"),
        os.path.join(images_dir, f"{stem}.png"),
        os.path.join(prompts_dir, f"{stem}.txt"),
    )
